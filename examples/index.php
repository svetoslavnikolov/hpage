 ...
 *
 *  VERSION   : 2.2, 29 Aug 2003 by Svetoslav Nikolov
 *              
 *
 */
...
$css_link = "http://www.oersted.dtu.dk/personal/sn/hpage/green.css";
$show_date = 0;
$page_look = "dtu";
...
//	if (substr($path,0,7)=='http://' || substr($path,0,8)=='https://')return;
	...
	while(false != ($file=readdir($top_dir_handle))){
		if (is_dir($path . $directory . $file) && $file[0]!="."){
			...
			$times[$no_subdirs] = filectime($file);
			$no_subdirs ++;
		}
	}
...
function make_menus_dtu()
{
  ...
  global $query_string;
   ...
   big_comment("Start of top and left menus");
	...
	print("</td></tr>")	;
	print("<tr>\n"); print("<td class=\"menu\">");
	print("<a class=\"navigation\" href=\"?$query_string|print\">Print Version</a><br>");
	print("<img src=\"$menu_line\" width=\"$line_width\" height=1 alt=\"menu line\">");  // Put \n for larger space
	print("</td>");
	print("</tr>\n");
		...
		while(!feof($in_file)){
			...
			if (strlen($line)>0){
				...
				if (strcmp($line,"<hpage_table>")==0){
					echo("<table cellpaddding=0 cellspacing=0 border=\"1px\" bordercolor= \"#aaaaaa\" width=\"100%\" >\n");
					$in_table= 1;
					$in_body = 0;
...
/*echo "<pre>";
	print_r($left_menu_name);
	print_r($left_menu_link);
	print_r($left_menu_path);
	print_r($menu_index);	
echo "</pre>";
*/
if ($action=="print"){
